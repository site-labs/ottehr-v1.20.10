#!/usr/bin/env node
// Usage: node prefix-zambdas.js --prefix=my-prefix [--dry-run]
...
if (!prefix) {
  console.error('Usage: node prefix-zambdas.js --prefix=your-prefix [--dry-run]');
  process.exit(1);
}
...
(async () => {
  ...
  for (const z of zambdas) {
    ...
    console.log((dryRun ? '[dry-run] ' : '') + `Updating ${name} -> ${newName}`);
    if (!dryRun) {
      ...
      try {
        ...
        results.push({ id, oldName: name, newName, updated });
      } catch (err) {
        console.error('Failed to update', name, err.message || err);
        results.push({ id, oldName: name, error: err.message || String(err) });
      }
    } else {
      results.push({ id, oldName: name, newName, dryRun: true });
    }
  }

  console.log('\nSummary:');
  for (const r of results) {
    if (r.skipped) console.log(' SKIP:', r.oldName);
    else if (r.dryRun) console.log(' DRY:', r.oldName, '->', r.newName);
    else if (r.error) console.log(' ERR :', r.oldName, r.error);
    else console.log(' OK  :', r.oldName, '->', r.newName);
  }
